...
if __name__ == "__main__":
    ...
    for i in range(len(abnormal_test)):
        ...
        for j in range(num_of_cf):
            ...
            # new_input = input_x + ratio * (cf - input_x)

            new_input = input_x + np.random.randn(*input_x.shape) * 0.001
            new_input = new_input.astype(np.float32)

            cf_ = gs.generate_counterfactual(new_input, eta)
            if len(cf_.shape) == 2:
    ...
    with open("synthetic_growingsphere.json", "w") as f:
