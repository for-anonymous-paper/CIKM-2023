...
if __name__ == "__main__":
    ...
    with open("./Hepatitis_cfmss.json", "r") as f:
    ...
    for i in range(len(abnormal_test)):
        ...
        # idx = np.random.randint(0, num_of_cf, size = 1)
        # ratio = np.random.uniform(0, 0.2, 1)
        # new_input = input_x + ratio * (cfs[idx] - input_x)
        new_input = input_x + np.random.randn(*input_x.shape) * 0.1
        new_input = new_input.astype(np.float32)

        cfs_ = dice.generate_counterfactuals(new_input, inital_points, num_of_cf)
        print(str(i), str(model.predict(cfs_)))
    ...
    with open("Hepatitis_dice.json", "w") as f:
