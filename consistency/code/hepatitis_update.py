...
def create_model_from_torch1(model_path):
    ...
    layer1 = Dense(30, input_shape = (10,), activation = 'relu', weights = [weight, bias])
    ...
    layer2 = Dense(10, input_shape = (30,), activation = 'relu', weights = [weight, bias])
    ...
    layer3 = Dense(2, input_shape = (10,), activation = 'softmax', weights = [weight, bias])
...
def create_model_from_torch(model_path):
    ...
    weight = torch_model[0].weight.detach().t().detach().numpy().astype(np.float32)
    bias = torch_model[0].bias.detach().numpy().astype(np.float32)
    layer1 = Dense(20, input_shape = (10,), activation = 'relu', weights = [weight, bias])

    weight = torch_model[2].weight.detach().t().detach().numpy().astype(np.float32)
    bias = torch_model[2].bias.detach().numpy().astype(np.float32)
    layer2 = Dense(10, input_shape = (20,), activation = 'relu', weights = [weight, bias])
...
if __name__ == "__main__":
    ...
    with open("../../cfgen/hepatitis/Hepatitis_cfmss.json", "r") as f:
    ...
    with open("hepatitis_sns_update.json", "w") as f:
